...
def calc_neg_log_lik(x_ND, log_pi_K, mu_KD, stddev_KD):
    ...
    # Initialize neg_log_lik to accumulate the log likelihoods
    neg_log_lik = 0.0

    # Iterate over each data point
    for n in range(N):
        # Compute the log likelihood of this data point under the GMM
        log_likelihoods = np.zeros(K)
        for k in range(K):
            # Compute the log likelihood of this data point under the k-th Gaussian component
            log_likelihoods[k] = stats.norm.logpdf(x_ND[n], loc=mu_KD[k], scale=stddev_KD[k]).sum() + log_pi_K[k]
        # Accumulate log likelihoods using logsumexp to avoid numerical issues
        neg_log_lik += -logsumexp(log_likelihoods)
