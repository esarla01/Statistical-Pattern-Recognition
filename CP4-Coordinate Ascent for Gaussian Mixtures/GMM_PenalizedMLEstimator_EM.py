...
class GMM_PenalizedMLEstimator_EM(GMM_PenalizedMLEstimator):
    ...
    * Initialization method generate_initial_parameters() from parent as well
    ...
    def mstep__update_mu_KD(self, r_NK, x_ND):
        ...
        mu_KD = np.dot(r_NK.T, x_ND)
        mu_KD /= 1e-13 + np.sum(r_NK, axis=0)[:,np.newaxis]
        return mu_KD
    ...
    def mstep__update_stddev_KD(self, r_NK, x_ND):
        ...
        denom_K = np.sum(r_NK, axis=0)

        # Initialize the array to store the updated standard deviations
        stddev_KD = np.zeros((self.K, self.D))
        ...
        for k in range(self.K):
            ...
            weighted_squared_diff_ND = np.dot(r_NK[:, k], (x_ND - self.mu_KD[k])**2)
            # Update the standard deviation using the formula
            stddev_KD[k] = np.sqrt(((1 / (s * m)) + weighted_squared_diff_ND) / (((1 / (s * m**2)) + np.sum(r_NK[:, k]))))
    ...
    def fit(self, x_ND, x_valid_ND=None, verbose=True):
        ...
        for iter_id in range(self.max_iter + 1):
            ...
            if x_valid_ND is None:
                ...
                va_score_per_pixel = self.score(x_valid_ND) / np.maximum(x_valid_ND.shape[0] * x_valid_ND.shape[1], 1.0)
