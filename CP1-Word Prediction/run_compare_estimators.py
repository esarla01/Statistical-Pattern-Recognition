...
if __name__ == '__main__':
    ...
    for i, n_train in enumerate(n_train_list):
        ...
        mapEst.fit(subset_train_word_list)
        map_scores[i] = (mapEst.score(test_word_list))

        # ---- fit PosteriorPredictive Estimator on train, then score it on test set
        ppEst = PosteriorPredictiveEstimator(vocab, alpha)
        ppEst.fit(subset_train_word_list)
        ppe_scores[i] = (ppEst.score(test_word_list))
