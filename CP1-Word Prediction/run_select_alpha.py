...
def calc_per_word_log_evidence(estimator, word_list):
    ...
    count_V = estimator.count_V

    log_numerator = gammaln(V * alpha) + np.sum(gammaln(count_V + alpha))

    log_denominator = gammaln(N + V * alpha) + np.sum(gammaln(np.ones(V) * alpha))


    log_evidence = log_numerator - log_denominator

    # if word_list == ["a", "a", "a", "a", "a", "a"]:
    #     import pdb;
    #     pdb.set_trace()
...
if __name__ == '__main__':
    ...
    for nn, N in enumerate(n_train_list):
        ...
        ax_grid[nn].set_xlabel('alpha value')
        if nn == 0:
            ax_grid[nn].set_ylabel('log likelihood')
    plt.tight_layout()
    plt.show()
