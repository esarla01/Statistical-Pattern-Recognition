...
class LinearRegressionMAPEstimator():
    ...
    def fit(self, x_ND, t_N):
        ...
        first_half = np.linalg.inv(np.dot(Phi_NM.T, Phi_NM) 
                                + (self.alpha / self.beta) * np.identity(M))
        second_half = np.dot(Phi_NM.T, t_N)

        self.w_map_M = np.dot(first_half, second_half)

        # Check the dimensions!
        assert self.w_map_M.shape == (M,)
    ...
    def predict(self, x_ND):
        ...
        return np.dot(phi_NM, self.w_map_M)
...
# # ... [rest of the LinearRegressionMAPEstimator class] ...
