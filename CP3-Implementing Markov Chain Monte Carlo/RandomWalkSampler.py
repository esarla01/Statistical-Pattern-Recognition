...
class RandomWalkSampler(object):
    ...
    def __init__(self, calc_tilde_log_pdf, list_of_rw_stddev_D=None, random_state=0):
    ...
    def draw_samples(self, zinit_D=None, n_keep_samples=1, n_burnin_samples=0):
        ...
        for s in range(K + B):
            ...
            for p, rw_stddev_D in enumerate(self.list_of_rw_stddev_D):
                ...
                log_pdf_current = self.calc_tilde_log_pdf(z_D)
                log_pdf_proposed = self.calc_tilde_log_pdf(zprime_D)
                A = np.exp(log_pdf_proposed - log_pdf_current)
